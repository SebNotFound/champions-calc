...
export function ImportDialog({ side, onClose, onImport, autoCapture }: Props) {
  ...
  // Overlay only: grab the connected device's screen via adb (Rust command) and
  // run the same recognizer straight away. Inert on the website (isTauri false).
          ...
          <>
            {isTauri() && (
              <button className="primary capture-device" onClick={captureFromDevice} disabled={busy}>
                {busy ? 'Reading the device…' : 'Capture from device (adb)'}
              </button>
            )}
            <div
              className="dropzone"
